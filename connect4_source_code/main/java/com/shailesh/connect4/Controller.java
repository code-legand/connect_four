...
public class Controller implements Initializable {
    ...
    private static String apposS = "'s";
    ...
    public void createPlayground(){
        setNamesButton.setOnAction(actionEvent -> setNames());
    ...
    private void setNames() {
        ...
        playerNameLabel.setText(isPlayerOneTurn ? PLAYER_ONE + apposS : PLAYER_TWO + apposS);
    }
...
//        run only after loading animation
        Platform.runLater(() -> {
    ...
    public void resetGame() {
        ...
        playerNameLabel.setText(PLAYER_ONE + apposS);

        createPlayground();
    }

    private boolean gameEnded(int currentRow, int column) {
        List<Point2D> verticalPoints = IntStream.rangeClosed(currentRow-3, currentRow+3).mapToObj(r -> new Point2D(r, column)).toList();
        List<Point2D> horizontalPoints = IntStream.rangeClosed(column-3, column+3).mapToObj(col -> new Point2D(currentRow, col)).toList();
